...
describe("MessageDraft", function () {
  ...
  test("should mention 2 users", async () => {
    ...
    expect((messagePreview[1].content as any).display).toBe(`@${user1.name}`)
    ...
    expect((messagePreview[3].content as any).display).toBe(`@${user2.name}`)
  ...
  test("should reference 2 channels", async () => {
    ...
    expect((messagePreview[1].content as any).display).toBe(`#${channel1.name}`)
    expect(messagePreview[2].type).toBe("text")
    expect(messagePreview[2].content.text).toBe(" and ")
    ...
    expect((messagePreview[3].content as any).display).toBe(`#${channel2.name}`)
