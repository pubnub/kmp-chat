 ...
 */
interface BaseChannel<C : BaseChannel<C, M>, M : BaseMessage<M, C>> {
    /**
    ...
    /**
     * Allows to update the [BaseChannel] metadata
     *
     ...
     *         in the metadata does not match any of the three default Chat SDK types.
     *
     * @return [PNFuture] containing the updated [BaseChannel] object with its metadata.
     */
    fun update(
        name: String? = null,
    ...
    /**
     * Allows to delete  an existing [BaseChannel] (with or without deleting its historical data from the App Context storage)
     *
     ...
     */
    fun delete(soft: Boolean = false): PNFuture<C?>

    /**
     * Forwards message to existing [BaseChannel]
     *
     * @param message Message object that you want to forward to the [BaseChannel].
     *
     * @return [PNFuture] containing [PNPublishResult] that holds the timetoken of the forwarded message.
     */
    fun forwardMessage(message: BaseMessage<*, *>): PNFuture<PNPublishResult>
    ...
    /**
     * Enables continuous tracking of typing activity within the [BaseChannel].
     *
    ...
    /**
     * Returns a list of users present on the [BaseChannel]
     *
    ...
    /**
     * Returns information if the user is present on the [BaseChannel]
     *
     * @param userId ID of the user whose presence you want to check.
     *
     * @return [PNFuture] with Boolean value informing if a given user is present on a specified [BaseChannel]
     */
    fun isPresent(userId: String): PNFuture<Boolean>

    /**
     *  Returns historical messages for the [BaseChannel]
     *
    ...
    fun sendText(
        ...
        ttl: Int? = null,
        quotedMessage: BaseMessage<*, *>? = null,
    ...
    /**
     * Watch the [BaseChannel] content without a need to [join] the [BaseChannel]
     *
     ...
     */
    fun connect(callback: (M) -> Unit): AutoCloseable
     ...
     */
    fun getPinnedMessage(): PNFuture<BaseMessage<*, *>?>

    /**
     * Attaches messages to the [BaseChannel]. Replace an already pinned message. There can be only one pinned message on a channel at a time.
     *
     * @param message that you want to pin to the selected channel.
     *
     * @return [PNFuture] containing [BaseChannel] with updated [BaseChannel.custom]
     */
    fun pinMessage(message: BaseMessage<*, *>): PNFuture<C>

    /**
     * Unpins a message from the [BaseChannel].
     *
     * @return [PNFuture] containing [BaseChannel] with updated [BaseChannel.custom]
     */
    fun unpinMessage(): PNFuture<C>
     ...
     */
    fun getMessage(timetoken: Long): PNFuture<M?>

    /**
     * Register a device on the [BaseChannel] to receive push notifications. Push options can be configured in [com.pubnub.chat.config.ChatConfiguration]
     *
    ...
    /**
     * Unregister a device from the [BaseChannel]
     *
     ...
     */
    fun streamUpdates(callback: (channel: C?) -> Unit): AutoCloseable
    ...
    /**
     * Enables real-time tracking of users connecting to or disconnecting from a [BaseChannel].
    ...
    /**
     * Fetches a list of reported message events for [BaseChannel] within optional time and count constraints.
     *
     ...
     */
    operator fun plus(update: PNChannelMetadata): C
